...
def test(x,args):
    ...
    for i in range(y.shape[0]):
        for j in range(len(args)/3):
            ...
            #y[i] += args[3*j] / (args[3*j+2] * math.sqrt(2 * math.pi)) * math.exp(-0.5 * ((x[i] - args[3*j+1]) / args[3*j+2]) ** 2)
            y[i] += args[3 * j] * math.exp(-0.5 * ((x[i] - args[3 * j + 1]) / args[3 * j + 2]) ** 2)
    return y

# [amplitude, x0, sigma]
par = np.array([4,10,8, 5,30,20, 10,70,15],np.float64)
initial = par*np.random.normal(1,0.5,par.shape[0])
print(par)
print(initial)
x = np.linspace(0,100,50,dtype=np.float64)
y = test(x,par)
...
min = np.array([0,np.min(x),1])
max = np.array([100,np.max(x),100])
bounds = np.array([ np.tile(min,par.shape[0]/3), np.tile(max,par.shape[0]/3) ],np.float64)
print(bounds.shape)
